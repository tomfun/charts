...
| `ssl.mtls.clientAuthType`              | When ssl.mtls.enabled is set, whether to accept client certificates not signed by one of the CAs specified in ssl.mtls.clientCaCerts. | RequireAndVerifyClientCert                                    |
| `ssl.mtls.clientCaCerts`               | When ssl.mtls.enabled is set, an array of client CA certificates, each in PEM format.                         | []                                                |
| `additionalPublicEntrypoints`          | Additional entrypoints that will be configured as http and https (and be public if so)                                       | {}                                                |
| `additionalPublicEntrypoints.$name`    | Additional entrypoint name, and for now also container and service port                                                      |                                                   |
...
| `dashboard.ingress.middlewares`        | Middlewares for the Traefik dashboard Ingress definition, specified as a map                                                 | None                                              |
